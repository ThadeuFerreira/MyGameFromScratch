...
MovePlayer(game_state *GameState, entity Entity, real32 dt, v2 ddP)
{
    ...
    /*
      uint32 MinTileX = Minimum(OldPlayerP.Chunk, NewPlayerP.Chunk);
      uint32 MinTileY = Minimum(OldPlayerP.ChunkY, NewPlayerP.ChunkY);
      uint32 MaxTileX = Maximum(OldPlayerP.Chunk, NewPlayerP.Chunk);
      uint32 MaxTileY = Maximum(OldPlayerP.ChunkY, NewPlayerP.ChunkY);
      ...
      uint32 ChunkZ = Entity.High->P.ChunkZ;
    */    
...
extern "C" GAME_UPDATE_AND_RENDER(GameUpdateAndRender)
{
    ...
    if(!Memory->IsInitialized)
    {
        ...
        NewCameraP.ChunkX = ScreenBaseX*TilesPerWidth + 17/2;
        NewCameraP.ChunkY = ScreenBaseY*TilesPerHeight + 9/2;
		NewCameraP.ChunkZ = ScreenBaseZ;
   ...
   if(CameraFollowingEntity.High)
        ...
        ++RelRow)
    {
        for(int32 RelColumn = -20;
            ...
            ++RelColumn)
        {
            uint32 Column = GameState->CameraP.Chunk + RelColumn;
            uint32 Row = GameState->CameraP.ChunkY + RelRow;
            ...
            if(TileID > 1)
            {
                ...
                if((Column == GameState->CameraP.Chunk) &&
                   (Row == GameState->CameraP.ChunkY))
                {
                    Gray = 0.0f;
                }
